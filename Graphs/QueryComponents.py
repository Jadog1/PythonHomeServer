    ...
    def simpleGraph(self, QueryResults, type=[], titles=[], ):
        ...
        for graph in QueryResults:
            if(type[i]=='pie'):
                ...
                f1_axes[i].pie(percentSum, labels=graph[1], autopct='%1.1f%%', normalize=True)
            elif(type[i]=='bar'):
                barResult=f1_axes[i].bar(graph[1], graph[0])
                f1_axes[i].bar_label(barResult, padding=3)
            else:
                if(len(graph)==2):
                    f1_axes[i].plot(graph[0], graph[1])
                else:
                    f1_axes[i].plot(graph[0])
            f1_axes[i].set_title(titles[i])
